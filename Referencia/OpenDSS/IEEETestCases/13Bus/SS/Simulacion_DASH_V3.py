...
for nombre in circuit.Lines.AllNames:
    ...
    #norm_amps = circuit.Lines.NormAmps or circuit.Lines.EmergAmps or 1.0  # evitar división por cero
...
# Reemplaza la sección de búsqueda binaria con este código:
...
for barra_gd in barras:
    ...
    if num_fases == 3:
        tipo_gd = "trifasica"
        kv = kv_ln * (3**0.5)  # kV línea-línea para GD trifásica
    elif num_fases == 2:
        tipo_gd = "bifasica"
        # Intenta obtener el voltaje entre fases directamente
        try:
            kv = circuit.ActiveBus.kVBase * 2  # Asume 180° entre fases
            # O alternativamente, si conoces el ángulo:
            # if angulo_entre_fases == 120:
            #     kv = kv_ln * (3**0.5)
        except:
            kv = kv_ln * 2  # Valor por defecto para split-phase
    else:
        tipo_gd = "monofasica"
        kv = kv_ln  # kV línea-neutro para GD monofásica
    ...
    while low <= high:
        ...
        # Aplicar GD según el tipo (código existente)
        ...
        # Verificación de voltaje (código existente)
        ...
        # Verificación de corriente y potencia
        ...
        for nombre, norm_amps, s_nom in lineas_info:
            ...
            mags = circuit.ActiveCktElement.CurrentsMagAng[::2]  # Magnitudes de corriente por fase
            
            # Obtener información de la línea
            bus1 = circuit.Lines.Bus1.split('.')[0]
            circuit.SetActiveBus(bus1)
            kv_base = circuit.ActiveBus.kVBase or 1.0  # kV base para la línea
            ...
            for i, mag in enumerate(mags):
                ...
                if fases_linea > 1:  # Para líneas multifásicas
                    s_real = (3**0.5) * kv_base * mag  # kVA trifásico
                else:  # Para líneas monofásicas
                    s_real = kv_base * mag  # kVA monofásico
