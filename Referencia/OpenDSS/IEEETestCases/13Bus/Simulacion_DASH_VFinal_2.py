...
# Variables globales que se inicializarán más tarde
engine = None
circuit = None
solution = None
text = None
...
voltajes_dict = None
barras_fase = None
barras_fases_disponibles = None
limites_gd = None
violaciones_corriente_por_barra_fase = None
lineas_info = None
perdidas_sin_gd_df = None
resumen_sin_gd = None
voltajes_df = None
limites_pivot = None
...
# Layout de bienvenida (solo tiene btn-iniciar)
layout_bienvenida = html.Div(
    style={
        "backgroundImage": "url('https://upload.wikimedia.org/wikipedia/commons/8/80/Power_lines_in_sunset.jpg')",
    ...
    children=[
        html.H1("Simulador de Generación Distribuida", style={"fontSize": "3rem", "marginBottom": "2rem"}),
        html.Button("Iniciar", id="btn-iniciar", className="btn btn-light btn-lg")
    ]
)

# Callback para mostrar la página correcta
@app.callback(
...
def display_page(pathname):
    if pathname == '/simulador':
        return crear_layout_simulador()
    return layout_bienvenida

# Callback para el botón iniciar (siempre presente)
@app.callback(
    Output('url', 'pathname'),
    Input('btn-iniciar', 'n_clicks'),
    prevent_initial_call=True
)
def ir_a_simulador(n_clicks):
    if n_clicks:
        return "/simulador"
...
def reiniciar_circuito():
    """Reinicia el circuito OpenDSS"""
    dss_folder = os.path.dirname(os.path.abspath(__file__))
    os.chdir(dss_folder)
    text.Command = "Clear"
...
def obtener_loss_table():
    """Calcula las pérdidas en el circuito"""
    elementos = []
...
def inicializar_opendss():
    """Función para inicializar OpenDSS y realizar cálculos iniciales"""
    global engine, circuit, solution, text, barras, voltajes_dict
    global barras_fase, barras_fases_disponibles, limites_gd
    global violaciones_corriente_por_barra_fase, lineas_info
    global perdidas_sin_gd_df, resumen_sin_gd, voltajes_df, limites_pivot
    ...
    # Pre-cache de información de líneas para verificación de corriente
    lineas_info = []
    for nombre in circuit.Lines.AllNames:
        circuit.Lines.Name = nombre
        fases = circuit.Lines.Phases
        norm_amps = circuit.Lines.EmergAmps or 1.0  # evitar división por cero
    ...
    # Búsqueda binaria optimizada para límites de GD
    for barra_gd in barras:
        ...
        for fase in nodos:
            ...
            while low <= high:
                ...
                # Verificación de voltaje optimizada
                ...
                # Verificación de corriente solo si no hay violación de voltaje
                ...
                for nombre, norm_amps, s_nom in lineas_info:
                    ...
                    for i, mag in enumerate(mags):
                        ...
                        if mag > norm_amps or s_real > s_nom:
                            violacion_corriente = violacion_corriente or (mag > norm_amps)
                            violacion_potencia = violacion_potencia or (s_real > s_nom)

                if not violacion_voltaje:
                    best_kw = mid
                    low = mid + 1
                else:
                    high = mid - 1
            ...
            # Guardar violaciones solo para el mejor caso
            if best_kw > 0:
                reiniciar_circuito()
                text.Command = f"New Generator.GD Bus1={barra_gd}.{fase} Phases=1 kV={kv_ln:.3f} kW={best_kw} kvar=0 Model=1"
                text.Command = "Solve"
                
                # Capturar violaciones para el mejor caso
                violaciones = []
                for nombre, norm_amps, s_nom in lineas_info:
                    circuit.Lines.Name = nombre
                    circuit.SetActiveElement(f"Line.{nombre}")
                    ...
                    for i, mag in enumerate(mags):
                        if mag > norm_amps:
                            violaciones.append({
                                ...
                                "Límite (A)": round(norm_amps, 2),
                                "Barra": barra_gd,
                                "Fase GD": fase
                            })
                
                if violaciones:
                    violaciones_corriente_por_barra_fase[(barra_gd, fase)] = violaciones
...
def crear_layout_simulador():
    """Función que crea el layout del simulador y registra sus callbacks"""
    # Inicializamos OpenDSS si no está inicializado
    if engine is None:
        inicializar_opendss()
    
    # Layout del simulador
    layout = html.Div([
        html.Button("Volver al menú", id="btn-volver", className="btn btn-secondary mb-3"),
        html.Div([
        ...
        html.Div([
            ...
            dcc.Input(id='potencia-gd', type='number', min=0, max=150000, value=100, className="form-control mb-3"),
    ...
    # Registramos los callbacks específicos del simulador
    @app.callback(
        Output('url', 'pathname'),
        Input('btn-volver', 'n_clicks'),
        prevent_initial_call=True
    )
    def volver_a_inicio(n_clicks):
        if n_clicks:
            return "/"
        return dash.no_update
    ...
    def actualizar_grafico(n_clicks, barra, fase_seleccionada, kw, tipo):
        ...
        try:
            ...
            for nombre, norm_amps, s_nom in lineas_info:
                ...
                bus1 = circuit.Lines.Bus1.split('.')[0]
                circuit.SetActiveBus(bus1)
                kv_base = circuit.ActiveBus.kVBase or 1.0  # evitar división por cero
                fases = circuit.Lines.Phases
                kv = kv_base * 1.732 if fases > 1 else kv_base
