...
# Funciones existentes (no necesitan cambios)
...
def ejecutar_simulacion(barra, potencia_kW):
    ...
    resultados = {
        "config": {"barra": barra, "potencia_kW": potencia_kW},
        "voltajes": [],
    ...
    for barra_name in voltajes_sin_gd.keys():
        v_sin = voltajes_sin_gd[barra_name].get(1, 0)  # Asumimos fase 1 por defecto
        v_con = voltajes_con_gd[barra_name].get(1, 0)
...
# Recibir parámetros desde Power BI
if __name__ == "__main__":
    params = json.loads(sys.argv[1])  # Recibe parámetros JSON desde Power BI
    barra = params["barra"]
    potencia_kW = params["potencia_kW"]
    
    # Ejecutar simulación
    resultados = ejecutar_simulacion(barra, potencia_kW)
    
    # Convertir los resultados a DataFrame para Power BI
    ...
    # Devolver resultados a Power BI
    print(json.dumps({
